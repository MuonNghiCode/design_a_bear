...
import { MdClose, MdOutlineInventory, MdTrendingUp, MdTrendingDown } from "react-icons/md";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-300">
        <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-black text-[#1A1A2E]">Điều chỉnh kho hàng</h2>
            <p className="text-xs font-bold text-[#6B7280]">{productName}</p>
          </div>
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-xl bg-gray-50 text-gray-400 hover:text-gray-600 transition-colors">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div className="space-y-1.5">
            <label className="text-[11px] font-black text-[#6B7280] tracking-wider uppercase">Chọn kho hàng</label>
            {isLoading ? (
          ...
          <div className="space-y-1.5">
            <label className="text-[11px] font-black text-[#6B7280] tracking-wider uppercase">Số lượng thay đổi</label>
            <div className="relative">
              ...
              <div className="absolute left-4 top-1/2 -translate-y-1/2">
                {delta >= 0 ? <MdTrendingUp className="text-xl text-green-500" /> : <MdTrendingDown className="text-xl text-red-500" />}
              </div>
            </div>
