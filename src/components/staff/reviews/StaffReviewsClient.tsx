...
const avg       = (ADMIN_REVIEWS.reduce((s, r) => s + r.rating, 0) / total).toFixed(1);
    ...
    <div ref={ref} className="space-y-5">
      ...
      <div className="ac flex items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <h1 className="font-black text-[#1A1A2E] text-2xl tracking-tight">Đánh giá</h1>
          </div>
          <p className="text-[#9CA3AF] text-sm">Trả lời đánh giá từ khách hàng</p>
        </div>
      </div>
      ...
      <div className="ac grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          <div key={label} className="bg-white rounded-2xl px-5 py-4 flex items-center gap-3">
            <div className="w-2.5 h-10 rounded-full shrink-0" style={{ backgroundColor: color }} />
            <div>
              <p className="text-[#9CA3AF] text-[10px] font-black tracking-wider uppercase">{label}</p>
