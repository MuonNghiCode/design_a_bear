...
const PILLS = [
  { icon: MdShoppingBag,  label: "Tổng đơn",    value: today,    color: "bg-white/15"       },
  { icon: MdAccessTime,   label: "Chờ xử lý",   value: pending,  color: "bg-[#FF8C42]/20"   },
  { icon: MdInventory,    label: "Đang đóng gói", value: packing, color: "bg-[#7C5CFC]/20"  },
  { icon: MdCheckCircle,  label: "Hoàn thành",   value: done,     color: "bg-[#4ECDC4]/20"   },
];
    ...
    <div className="relative bg-[#17409A] rounded-3xl overflow-hidden p-6 sm:p-8 flex flex-col gap-5 min-h-56">
      ...
      <div className="relative flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="shrink-0 flex flex-col items-center justify-center w-20 h-20 rounded-2xl bg-[#17409A]/40 border border-white/20 backdrop-blur-sm">
          <span className="text-white font-black text-xl leading-tight">{shipping}</span>
          <span className="text-white/60 text-[10px] font-semibold">vận chuyển</span>
        </div>
      </div>
      ...
      <div className="relative flex flex-col gap-1.5">
        {[
          ...
          { label: "Vận chuyển",   value: shipping, max: today, color: "#17409A" },
          { label: "Hoàn thành",   value: done,     max: today, color: "#4ECDC4" },
        ].map(({ label, value, max, color }) => (
          <div key={label} className="flex items-center gap-2">
            <span className="text-white/55 text-[11px] font-semibold w-24 shrink-0">{label}</span>
            ...
            <span className="text-white/50 text-[11px] font-semibold w-5 text-right">{value}</span>
          </div>
        ))}
      </div>
    </div>
