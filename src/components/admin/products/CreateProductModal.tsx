...
export default function CreateProductModal({
  ...
  // Per Backend Docs: EXCLUDE AI_PROCESSOR from image matrix combinations
  const matrixAccessoryIds = useMemo(() => {
    return selectedAccessoryIds.filter(id => {
      ...
      // Exclude if type is AI_PROCESSOR or name contains "AI PROCESSOR" (robust check)
  ...
  const allCombinations = useMemo(() => {
    ...
    // EXCLUDE the empty set (Base Bear only) as per user request
  ...
  const handleUploadImage = async () => {
    ...
    try {
      ...
      if (!res.isSuccess || !res.value?.publicUrl) throw new Error("Upload ảnh thất bại");
  ...
  const handleFinish = async () => {
    ...
    setInternalIsSubmitting(true);
    try {
      // 1. Create Default Personalization Group
      ...
      // 2. Create Rules for each accessory
      ...
      // 3. Update product with combo images
          ...
          {step === 1 ? (
             <form id="createProductForm" onSubmit={handleStep1Submit} className="space-y-6">
                ...
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                  <label className="text-[11px] font-black text-[#6B7280] tracking-wide uppercase">Ảnh chính sản phẩm (Gấu gốc)</label>
                  <div className="flex items-center gap-4">
                    <div className="w-20 h-20 rounded-2xl bg-[#F9FAFB] border-2 border-dashed border-gray-200 flex items-center justify-center overflow-hidden shrink-0">
                      {formData.imageUrl ? <img src={formData.imageUrl} className="w-full h-full object-cover" /> : <MdCloudUpload className="text-gray-300 text-2xl" />}
                    </div>
                    <div className="flex-1 space-y-2">
                       <input type="file" accept="image/*" onChange={(e) => setUploadFile(e.target.files?.[0] ?? null)} className="block w-full text-xs text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-[#17409A]/10 file:text-[#17409A] hover:file:bg-[#17409A]/20" />
                       <button type="button" onClick={handleUploadImage} disabled={!uploadFile || isUploadingImage} className="w-full py-2.5 rounded-xl text-xs font-bold bg-[#17409A] text-white hover:bg-[#0E2A66] disabled:opacity-50 transition-all">{isUploadingImage ? "Đang xử lý..." : "Upload ảnh ngay"}</button>
                    </div>
                  </div>
                </div>
                         ...
                         return (
                            <button key={acc.productId} onClick={() => setSelectedAccessoryIds(prev => isSelected ? prev.filter(id => id !== acc.productId) : [...prev, acc.productId])} className={`flex items-center gap-3 p-3 rounded-xl border-2 transition-all ${isSelected ? 'border-[#17409A] bg-[#F4F7FF]' : 'border-white bg-white hover:border-gray-200'}`}>
                               ...
                               <div className="flex-1 text-left min-w-0">
                                  ...
                                  <p className="text-[10px] font-bold text-[#9CA3AF]">{formatPrice(acc.price)}</p>
                               </div>
