        ...
        <div className="p-6 overflow-y-auto custom-scrollbar space-y-8">
              ...
              <div className="aspect-square rounded-2xl bg-white border border-[#E5E7EB] overflow-hidden shadow-sm flex items-center justify-center p-2 relative">
                ...
                {!product.isActive && (
                  <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                    <span className="bg-white/90 text-[#EF4444] px-3 py-1 rounded-full text-[10px] font-black uppercase shadow-lg">
                      Đã ẩn
                    </span>
                  </div>
                )}
            ...
            <div className="space-y-5">
              ...
              <div className="flex flex-wrap gap-2">
                {product.categories?.map((c: any) => (
                  <span
                    key={c.categoryId}
                    className="px-2.5 py-1 rounded-lg bg-[#7C5CFC]/10 text-[#7C5CFC] text-[10px] font-black uppercase tracking-wider"
                  >
                    {c.name}
                  </span>
                ))}
                {product.characters?.map((c: any) => (
                  <span
                    key={c.characterId}
                    className="px-2.5 py-1 rounded-lg bg-[#FF8C42]/10 text-[#FF8C42] text-[10px] font-black uppercase tracking-wider"
                  >
                    {c.name}
                  </span>
                ))}
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                <div className="bg-white rounded-2xl p-3 border border-[#E5E7EB] shadow-sm">
                  <p className="text-[#9CA3AF] text-[9px] font-black tracking-widest uppercase mb-1">
                    Trạng thái
                  </p>
                  <p
                    className={`text-[10px] font-black mt-1 uppercase ${product.isActive ? "text-[#4ECDC4]" : "text-[#EF4444]"}`}
                  >
                ...
                <div className="bg-white rounded-2xl p-3 border border-[#E5E7EB] shadow-sm">
                  ...
                  <p
                    className={`text-[10px] font-black mt-1 uppercase ${product.isPersonalizable ? "text-[#4ECDC4]" : "text-[#EF4444]"}`}
                  >
              ...
              {/* Aggregated Inventory Quick Link */}
              <div className="bg-[#17409A]/5 border border-[#17409A]/10 p-4 rounded-2xl flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-[#17409A] border border-[#17409A]/10 shrink-0">
                    <MdGridView className="text-xl" />
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-[#17409A] uppercase tracking-wider">
                      Tổng sản phẩm khả dụng
                    </p>
                    <p className="text-lg font-black text-[#17409A] leading-tight">
                      {product.available || 0}{" "}
                      <span className="text-[10px] font-bold opacity-60 ml-0.5 whitespace-nowrap">
                        đơn vị sẵn sàng bán
                      </span>
                    </p>
                  </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-gray-50/80 border-b border-gray-100">
                        ...
                        <th className="px-4 py-3 text-[10px] font-black text-gray-500 uppercase text-center">
                          Thực tế
                        </th>
                        <th className="px-4 py-3 text-[10px] font-black text-gray-500 uppercase text-center">
                          Giữ chỗ
                        </th>
                        <th className="px-4 py-3 text-[10px] font-black text-[#17409A] uppercase text-center bg-[#17409A]/5">
                          Khả dụng
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                      {product.variants.map((v) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <span className="text-[10px] font-mono bg-gray-100 px-1.5 py-0.5 rounded uppercase">
                              {v.sku}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3 text-center">
                            <span
                              className={`text-xs font-bold ${v.reserved > 0 ? "text-orange-500" : "text-gray-300"}`}
                            >
                              {v.reserved}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-center bg-[#17409A]/5">
                            <span className="text-sm font-black text-[#17409A]">
                              {v.available}
                            </span>
                          </td>
                        </tr>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                  {product.comboImages.map((ci) => (
                    <div
                      key={ci.comboId}
                      className="bg-white rounded-2xl border border-[#E5E7EB] p-2 space-y-2 group hover:shadow-lg transition-all duration-300"
                    >
                      ...
                      <p
                        className="text-[9px] font-black text-[#1A1A2E] truncate px-1 uppercase tracking-tight"
                        title={ci.combinationKey}
                      >
                        {ci.combinationKey}
                      </p>
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-3">
              ...
              <div className="bg-white/50 rounded-2xl p-5 border border-white/50 text-sm font-semibold text-[#4B5563] leading-relaxed italic h-full min-h-[120px]">
            ...
            {product.accessories && product.accessories.length > 0 && (
              <div className="space-y-3">
                <h4 className="text-[11px] font-black text-[#6B7280] tracking-[0.2em] uppercase pl-1">
                  Phụ kiện đi kèm ({product.accessories.length})
                </h4>
                <div className="bg-white border border-gray-100 rounded-2xl p-3 space-y-2 max-h-[120px] overflow-y-auto custom-scrollbar">
                  {product.accessories.map((acc: any) => (
                    <div
                      key={acc.accessoryId}
                      className="flex items-center gap-2 p-2 rounded-xl bg-gray-50/50 border border-gray-50"
                    >
                      <img
                        src={acc.imageUrl}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[9px] text-[#17409A] font-bold">
                          {formatPrice(acc.targetPrice)}
                        </p>
                      </div>
                    </div>
                  ))}
          ...
          <div className="grid grid-cols-2 gap-4 pb-4">
            ...
            <div className="text-right">
              ...
              <p className="text-sm font-black text-[#17409A] mt-1">
                {product.productType}
              </p>
            </div>
          </div>
