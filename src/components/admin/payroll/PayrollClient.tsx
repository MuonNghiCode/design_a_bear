...
export default function PayrollClient() {
  ...
  return (
    <div className="flex flex-col gap-8 pb-12" style={{ fontFamily: "var(--font-nunito), Nunito, sans-serif" }}>
      {/* Page Header Area */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-[#1A1A2E] font-black text-3xl tracking-tight">Quản Lý Lương & Hoa Hồng</h1>
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{
                    backgroundColor: kpi.primary ? "rgba(255,255,255,0.15)" : kpi.accent + "15",
                  }}
                >
                  <Icon className="text-2xl" style={{ color: kpi.primary ? "#fff" : kpi.accent }} />
                </div>
                <div>
                  <p className={`text-[10px] font-black tracking-[0.25em] uppercase ${kpi.primary ? "text-white/60" : "text-[#9CA3AF]"}`}>
                    {kpi.label}
                  </p>
      ...
      <div className="bg-white rounded-[40px] shadow-sm border border-[#F0F0F8] overflow-hidden">
        ...
        {loading ? (
          <div className="py-32 flex flex-col items-center gap-4 text-[#9CA3AF]">
            ...
            <p className="font-black text-sm uppercase tracking-widest">Đang kết xuất dữ liệu...</p>
          </div>
        ) : entries.length === 0 ? (
          <div className="py-32 flex flex-col items-center gap-4 text-[#9CA3AF]">
            <p className="font-black text-sm uppercase tracking-widest italic">Không có dữ liệu trong kỳ này</p>
          </div>
        ) : (
            ...
            {entries.map((entry, idx) => {
              ...
              return (
                <div key={entry.userId}>
                  ...
                  >
                    <span className="text-[#9CA3AF] text-sm font-black tracking-tight">{idx + 1}</span>
                    ...
                    <div className="flex items-center gap-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-[#1A1A2E] font-black text-sm truncate uppercase tracking-tight">{entry.fullName}</p>
                        <span className="inline-block mt-1 px-2.5 py-0.5 rounded-full text-[9px] font-black tracking-widest uppercase" style={{ color: roleCfg.color, backgroundColor: roleCfg.bg }}>
                          {roleCfg.label}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="hidden md:block text-center">
                      <p className="text-[#17409A] font-black text-xl">{entry.completedJobs}</p>
                    </div>

                    <div className="hidden md:block text-right">
                      <p className="text-[#1A1A2E] font-bold text-sm">{formatVND(entry.craftsmanCommission)}</p>
                    </div>

                    <div className="hidden md:block text-right">
                      <p className="text-[#1A1A2E] font-bold text-sm">{formatVND(entry.qcCommission)}</p>
                    </div>

                    <div className="text-right">
                      <p className="text-[#17409A] font-black text-base">{formatVND(entry.totalEarned)}</p>
                    </div>

                    <div className="flex justify-end">
                      <MdExpandMore className={`text-[#9CA3AF] text-2xl transition-transform duration-300 ${isOpen ? "rotate-180" : ""}`} />
                    </div>
                  </button>
                  ...
                  {isOpen && (
                    <div className="bg-[#F8FAFF] px-10 py-6 border-t border-[#F0F0F8]">
                      {entry.jobs.length === 0 ? (
                        <p className="text-[#9CA3AF] text-sm font-bold italic py-8 text-center">Không có bản ghi công việc chi tiết.</p>
                      ) : (
                          ...
                          <table className="w-full text-sm">
                            <thead>
                              <tr className="text-[#9CA3AF] text-[9px] font-black uppercase tracking-[0.25em]">
                                <th className="text-left py-3 px-4">Mã công việc</th>
                                <th className="text-left py-3 px-4 hidden md:table-cell">Sản phẩm</th>
                                <th className="text-left py-3 px-4 hidden lg:table-cell">Ngày hoàn tất</th>
                            ...
                            <tbody className="divide-y divide-[#F8F9FF]">
                              {entry.jobs.map((job) => (
                                <tr key={job.jobId} className="hover:bg-[#F8FAFF] transition-colors">
                                  <td className="py-4 px-4 font-mono text-[11px] text-[#6B7280]">{job.jobId.slice(0, 12).toUpperCase()}...</td>
                                  <td className="py-4 px-4 text-[#1A1A2E] font-bold hidden md:table-cell">{job.productName || "Gấu Bông"}</td>
                                  <td className="py-4 px-4 text-[#6B7280] text-xs hidden lg:table-cell">
                                    {job.completedAt ? format(new Date(job.completedAt), "dd/MM/yyyy", { locale: vi }) : "—"}
                                  </td>
                                  <td className="py-4 px-4 text-right font-bold text-[#17409A]">{formatVND(job.craftsmanCommission || 0)}</td>
                                  <td className="py-4 px-4 text-right font-bold text-[#7C5CFC]">{formatVND(job.qcCommission || 0)}</td>
                                  <td className="py-4 px-4 text-right font-black text-[#1A1A2E]">
                                    {formatVND((job.craftsmanCommission || 0) + (job.qcCommission || 0))}
                                  </td>
                                </tr>
                              ))}
                            </tbody>
