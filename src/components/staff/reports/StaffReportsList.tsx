...
function DetailModal({ report, onClose }: { report: StaffReport; onClose: () => void }) {
        ...
        <div className="bg-[#17409A] px-6 py-5 flex items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-xs font-black px-2.5 py-0.5 rounded-xl" style={{ color: typeCfg.color, backgroundColor: `${typeCfg.color}30` }}>
                {typeCfg.label}
              </span>
              <span className="text-xs font-black px-2.5 py-0.5 rounded-xl" style={{ color: statusCfg.color, backgroundColor: `${statusCfg.color}30` }}>
                {statusCfg.label}
              </span>
            </div>
            <p className="text-white font-black text-lg leading-snug">{report.title}</p>
            <p className="text-white/60 text-xs mt-0.5">{report.staffName} · {report.date} · {shiftCfg.label}</p>
          </div>
          <button onClick={onClose} className="shrink-0 w-8 h-8 rounded-xl bg-white/15 hover:bg-white/25 flex items-center justify-center text-white transition-colors cursor-pointer">
        ...
        <div className="overflow-y-auto flex-1 p-6 flex flex-col gap-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <div key={label} className="bg-[#F4F7FF] rounded-2xl px-4 py-3 text-center">
                <p className="text-[#9CA3AF] text-[10px] font-semibold uppercase tracking-wide">{label}</p>
                <p className="text-[#1A1A2E] font-black text-xl mt-0.5">{value}</p>
              </div>
            ))}
          </div>
          ...
          {report.issues.length > 0 && (
            <div className="flex flex-col gap-2">
              ...
              {report.issues.map((issue) => {
                ...
                return (
                  <div key={issue.id} className="bg-[#F4F7FF] rounded-2xl p-4 flex items-start gap-3">
                    <span className="shrink-0 text-[10px] font-black px-2 py-0.5 rounded-lg mt-0.5" style={{ color: sev.color, backgroundColor: sev.bg }}>
                      {sev.label}
                    </span>
                    <p className="text-[#374151] text-sm flex-1">{issue.description}</p>
                    {issue.resolved ? (
                      <MdCheckCircle className="text-[#4ECDC4] shrink-0 text-lg" title="Đã giải quyết" />
                    ) : (
                      <MdWarning className="text-[#FF8C42] shrink-0 text-lg" title="Chưa giải quyết" />
                    )}
                  </div>
                );
              })}
          ...
          {report.stockNotes && (
            <div className="flex flex-col gap-1.5">
              <p className="font-black text-[#1A1A2E] text-sm">Tình trạng kho</p>
              <div className="bg-[#F4F7FF] rounded-2xl p-4">
                <p className="text-[#374151] text-sm leading-relaxed">{report.stockNotes}</p>
              </div>
            </div>
          )}
          ...
          {report.handoverNotes && (
            <div className="flex flex-col gap-1.5">
              <p className="font-black text-[#1A1A2E] text-sm">Ghi chú bàn giao</p>
              <div className="bg-[#FFD93D]/10 border border-[#FFD93D]/30 rounded-2xl p-4">
                <p className="text-[#374151] text-sm leading-relaxed">{report.handoverNotes}</p>
              </div>
            </div>
          )}
          ...
          {report.generalNotes && (
            <div className="flex flex-col gap-1.5">
              ...
              <div className="bg-[#F4F7FF] rounded-2xl p-4">
                <p className="text-[#374151] text-sm leading-relaxed">{report.generalNotes}</p>
              </div>
            </div>
          )}
          ...
          {report.reviewNote && (
            <div className="flex flex-col gap-1.5">
              <p className="font-black text-[#1A1A2E] text-sm">Phản hồi từ Admin</p>
              <div className="bg-[#17409A]/10 border border-[#17409A]/20 rounded-2xl p-4">
                <p className="text-[#17409A] text-xs font-semibold mb-1">{report.reviewedBy}</p>
                <p className="text-[#374151] text-sm leading-relaxed">{report.reviewNote}</p>
              </div>
            </div>
          )}
        </div>
    ...
    <>
      <div className="bg-white rounded-3xl overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-[#F4F7FF]">
          ...
          <p className="text-[#9CA3AF] text-xs mt-0.5">{STAFF_REPORTS.length} báo cáo</p>
        </div>
              ...
              <div key={r.id} className="group">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-[#1A1A2E] text-sm truncate">{r.title}</p>
                    <p className="text-[#9CA3AF] text-xs">
                      {r.date} · <span style={{ color: shiftCfg.color }}>{shiftCfg.label}</span>
                      {r.issuesCount > 0 && (
                        <span className="ml-1.5 text-[#FF8C42]">· {r.issuesCount} sự cố</span>
                      )}
                    </p>
                  </div>
                ...
                {isOpen && (
                  <div className="px-6 pb-4 grid grid-cols-1 sm:grid-cols-3 gap-3 bg-[#F4F7FF]/40">
                    {[
                      ...
                      <div key={label} className="bg-white rounded-xl px-4 py-2.5 flex items-center justify-between">
                        <span className="text-[#9CA3AF] text-xs">{label}</span>
                        <span className="text-[#1A1A2E] font-black text-sm">{value}</span>
                      </div>
                    ))}
                    {r.handoverNotes && (
                      <div className="sm:col-span-3 bg-[#FFD93D]/10 rounded-xl px-4 py-2.5">
                        <p className="text-[#e6a800] text-[10px] font-black uppercase tracking-wide mb-0.5">Bàn giao</p>
                        <p className="text-[#374151] text-xs line-clamp-2">{r.handoverNotes}</p>
                      </div>
                    )}
                  </div>
                )}
